...
Most field data experts (level 1) will never become scripts users, they need simple (or _simplified_) __point & click__ interfaces!
...
## Follow the guide: Let's add a module with to have an UI for our function

Running the function `level_up(name = "my_first_module")` will add a new module - called "my_first_module" (rename it as you wish- trying to use a meaningful name) - to your dashboard 
...
The body and sidebar lines need to be separated with commas if you have multiple modules, but the app_server will be individual lines.
...
>  Be cautious with password and token - do not upload this in github. Password and token should rather be called as environement variables and set up separately after deployement in rstudio Connect
