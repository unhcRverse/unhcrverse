...
## Package on CRAN - Ideal but not compulsory!

[CRAN (The Comprehensive R Archive Network)](https://cran.r-project.org/web//packages//submission_checklist.html) offer a place to publish packages. it comes with a process of checking and verifying that an R package meets the necessary standards and guidelines.

The package should address a relevant and current need within the R user community. If there are similar packages available, the new package should offer improvements, additional features, or a different approach.

Not all R packages are submitted and available in CRAN. While CRAN publication, guarantees reliability and functionality for users, getting a package there, especially if it includes a complex pipeline might be lengthy and complicated, especially at development stage and/or when a package is developed for use within a single specific organization..
...
## Common issues to fix: font  not found

The package is not able to use UNHCR standard font __Lato__
...
Warning in grid.Call(C_textBounds, 
  ...
  font family 'Lato' not found in 
  PostScript font database
```

Let's add the required font within your package / put them in a folder [`inst/font'](https://github.com/Edouard-Legoupil/graveler/tree/main/inst/extdata/inst/fonts)

Then create a file [`R/load_font.R`](https://github.com/Edouard-Legoupil/GravelerDemo/blob/master/R/load_font.R) to load the font from your package when your package is loaded.
...
## Common issues to fix: NOTE Missing Import
...
Revise all the function used in your function and make sure they are correctly referenced within the function documentation
...
## Common issues to fix: NOTE visible binding for global variable


```{r eval=FALSE}
❯ checking R code for possible problems ... NOTE
  plot_top10_by_country_and_year: no visible binding for global variable
    ‘coa_name’
```

Manage environment variable within package  with `{checkhelper}`

```{r eval=FALSE}
# remotes::install_github("thinkr-open/checkhelper")
checkhelper::print_globals() 

## save the output in globals.R
globalVariables(unique(c(
  # plot_top10_by_country_and_year: 
  "coa_name", "coo_name", "oip", "refugees" 
)))
```

---

## Common issues to fix: WARNING: ASCII
...
❯ checking R files for non-ASCII characters 
   ... WARNING
  Found the following file with 
  non-ASCII characters:
    
    plot_top10_by_country_and_year.R
...
```{r eval=FALSE}
 "Source: Refugee Data Finder
  <br>\u00a9 UNHCR, The UN Refugee Agency"  #<<
```
]
.pull-right[
...
You can use [AI/Copilot](https://copilot.microsoft.com/) to build your unit testing!!!
...
# Reference
 ...
 - The non fusen way... [How to write your own R package and publish it on CRAN](https://www.mzes.uni-mannheim.de/socialsciencedatalab/article/r-package/)
